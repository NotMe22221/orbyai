// background.js — Resident Secretary Service Worker v1.1
// Fixes: detectPageType inlined in page context, toggle-overlay command,
//        programmatic injection fallback, per-tab state tracking,
//        forms+buttons extraction for Agent B context.
...
// ── Toolbar click + keyboard command ────────────────────────────────────────
...
async function toggleOverlay(tab) {
  ...
  if (
    ...
    console.warn('[RS] Cannot inject into restricted URL:', tab.url);
    return;
  }
  ...
  if (!isActive) {
    ...
    try {
      ...
      console.error('[RS] Failed to show overlay:', err.message);
    ...
    } catch { /* content script may have been unloaded */ }
  }
}

// Inject content script if not already loaded (programmatic fallback)
...
async function getPageContext(tab) {
  try {
    const result = await chrome.scripting.executeScript({
      target: { tabId: tab.id },
      func: () => {
        // IMPORTANT: all helper functions must be inlined here —
        // the service worker scope is not accessible inside executeScript.
        ...
        // Collect visible form fields for Agent B
...
async function handleVoiceCommand(payload, tab) {
  ...
  // STT: convert audio to transcript if not already provided
  if (!transcript && payload.audioBase64) {
    ...
    recordingId = sttData.recordingId || null;
  }

  if (!transcript.trim()) throw new Error('Empty transcript — nothing to process');
...
async function handleAnalyzeScreen(tab) {
  ...
  if (!screenshotBase64) {
    return { responseText: 'Screenshot capture failed. Check extension permissions.' };
  }
  ...
  const res = await fetch(`${backend}/api/voice`, {
    ...
    body: JSON.stringify({
      sessionId: currentSessionId || generateSessionId(),
      transcript:
        'Analyze this screen: describe the main content, key interactive elements (forms, buttons, links), the page purpose, and suggest the single most useful action I could take right now.',
      pageContext: context,
      vapiRecording: null,
      screenshotBase64,
    }),
